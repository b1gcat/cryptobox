...
func verifyHashId(_ fyne.Window) {
	...
	pub := widget.NewEntry()
	pub.SetText("e95001aed4b6f7de59169913997dace404f05091ed49c37133a9950a69405a9c")
	v0 := container.NewBorder(layout.NewSpacer(), layout.NewSpacer(),
		widget.NewLabel("待识别数据(hex)"), layout.NewSpacer(), pub)
	...
	verify = widget.NewButton("开始识别", func() {
		...
		info, err := Identify([]byte(pub.Text),
		...
		for _, i := range info {
			...
			result.Append(fmt.Sprintf("[+] %s(%v byte)\n", i.Name(), len(pub.Text)))
		}

	})

	box := container.NewVBox(v0, verify, result)
...
func GetDefaultPrototypes() []Prototype {
	return []Prototype{
		...
		},
		{
			...
			[]HashInfo{
				{ID: SM3, Hashcat: "?", John: "?", Extended: false},
